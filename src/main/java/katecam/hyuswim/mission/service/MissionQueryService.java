// src/main/java/katecam/hyuswim/mission/service/MissionQueryService.java
...
public class MissionQueryService {
    ...
    /** 오늘 날짜 기준으로 모든 미션을 가져오고, 각 미션의 오늘 통계(started/completed)와 특정 사용자(userId)의 오늘 상태를 함께 얹어 반환. */
    public List<MissionTodayResponse> getTodayMissionsWithState(Long userId) {
        ...
        return all.stream()
                .map(m -> {
                    MissionTodayResponse dto = MissionTodayResponse.of(m);
                    ...
                    if (myToday == null) {
                        dto.setState(TodayState.NOT_STARTED);
                    } else if (!Boolean.TRUE.equals(myToday.getIsCompleted())
                            && myToday.getMission().getId().equals(m.getId())) {
                        dto.setState(TodayState.IN_PROGRESS);
                    } else if (Boolean.TRUE.equals(myToday.getIsCompleted())
                            && myToday.getMission().getId().equals(m.getId())) {
                        dto.setState(TodayState.COMPLETED);
                    } else {
